...
| schedule_tags | VARCHAR(255) | - | Теги расписания, через запятую (В коде обрабатывать как список) 
...
| priority | VARCHAR(20) | + | Приоритет ячейки. Возможные значения: [LOW, NORMAL, HIGH, ONE IN A WEEK, SINGULAR]. Вместо null всегда заносить NORMAL |
### Период расписания
| Поле | Тип | Обязательность | Описание |
| ------ | ------ | ------ | ------ |
| id | VARCHAR(32) | + | Идентификатор. Заполняется при создании автоматически. UUID без разделителей |
...
3) Для периодов должен быть метод получения с фильтром и сортировкой
...
6) При получении по названию, принимать так же варианты, с ошибками не более чем в двух буквах